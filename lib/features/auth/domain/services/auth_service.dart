...
class AuthService implements AuthServiceInterface {
  ...
  Future<ResponseModel> login(
    ...
    if (response.statusCode == 200) {
      ...
      if (isCustomerVerificationOn && response.data['is_phone_verified'] == 0) {
      } else {
        authRepositoryInterface.saveUserToken(response.data['token']);
        await authRepositoryInterface.updateToken();
        authRepositoryInterface.clearSharedPrefGuestId();
      }
      responseModel = ResponseModel(true,
          '${response.data['is_phone_verified']}${response.data['token']}',
          isPhoneVerified: response.data['is_phone_verified'] == 1);
    } else {
  ...
  Future<bool> loginWithSocialMedia(SocialLogInBody socialLogInBody,
    ...
    if (response.statusCode == 200) {
      ...
      if (token != null && token.isNotEmpty) {
        if (isCustomerVerificationOn &&
            response.data['is_phone_verified'] == 0) {
          if (Get.find<SplashController>()
              .configModel!
              .firebaseOtpVerification!) {
            Get.find<AuthController>().firebaseVerifyPhoneNumber(
                response.data['phone'], token,
                fromSignUp: true);
          } else {
            Get.toNamed(RouteHelper.getVerificationRoute(
                response.data['phone'] ?? socialLogInBody.email,
                token,
                RouteHelper.signUp,
                ''));
          }
        ...
        response.data['errors'][0]['code'] == 'email') {
      Get.toNamed(RouteHelper.getForgotPassRoute(true, socialLogInBody));
    } else {
      showCustomSnackBar(response.statusMessage);
    }
